...
def main() -> None:
    ...
    # hello_numerai.pkl (basic model upload)
    hello = BasePredictor(
        models,
        stacker,
        aggregator,
    ...
    # feature_neutralization.pkl (neutralize to all features for now)
    feature_neutral = FeatureNeutralPredictor(
        ...
        do_rank=do_rank,
        rank_by_era=rank_by_era,
        neutralize_cols=feature_cols,
        proportion=1.0,
    )

    # target_ensemble.pkl (ensemble only lgb seed models if present)
    lgb_seed_models = [name for name in models.keys() if name.startswith("lgb_s")]
    if not lgb_seed_models:
        lgb_seed_models = list(models.keys())
    target_ensemble = BasePredictor(
        models,
        stacker=None,
        aggregator="mean",
    ...
    (out_dir / "target_ensemble.pkl").write_bytes(serializer.dumps(target_ensemble))

    print(f"Saved: {out_dir / 'hello_numerai.pkl'}")
    print(f"Saved: {out_dir / 'feature_neutralization.pkl'}")
    print(f"Saved: {out_dir / 'target_ensemble.pkl'}")
